<?php 
	...
	//	print_r($data['query']['result']);
		...
		public function deleteTrip($tempatWisata)
		{
			...
			$data['halte_awal']="Taman Mini Garuda";
			...
			else if($indexYangDihapus == $indexAwal)
			{
				...
				$data['query'] = $this->touristAttrManager->getAllTour2($data);
				$counter = 0;
			...
			else
			{
				...
				{
					$buswayPriceArray[$indexKanan]="0";
					$angkotPriceArray[$indexKanan]="0";
					// echo json_encode($buswayPriceArray);
					// echo json_encode($angkotPriceArray);
				}
				$tempHargaAkhir=  intval($buswayPriceArray[$indexKanan]) + intval($angkotPriceArray[$indexKanan])+ intval($ticketPriceArray[$indexKanan]);
